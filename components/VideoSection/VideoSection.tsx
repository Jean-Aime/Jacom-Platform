    ...
    <section className="py-20 px-6 md:px-20 bg-white">
      <div className="grid md:grid-cols-2 gap-12 items-center max-w-6xl mx-auto animate-fade-in">
        <div className="bg-gray-900 p-12 rounded relative">
          <div className="absolute top-8 left-8 bg-white px-4 py-3 rounded-lg text-sm max-w-xs animate-fade-up">
        ...
        <div className="relative bg-gray-200 h-96 rounded flex items-center justify-center overflow-hidden hover-scale">
          <Image
            src={videoImage}
            alt="Executive video"
            fill
          ...
          />
          <button className="relative z-10 w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-lg hover:scale-110 active:scale-90 transition-transform">
            <span className="text-primary text-2xl ml-1">▶</span>
          </button>
          
          <div className="absolute bottom-8 left-8 right-8 text-white animate-fade-up">
            <p className="text-xs mb-2">{personName}</p>
            <h3 className="text-2xl font-light mb-4">{videoTitle}</h3>
            <a href="#" className="text-sm underline hover:translate-x-1 inline-block transition-transform">
              {videoLink}
            </a>
          </div>
        </div>
