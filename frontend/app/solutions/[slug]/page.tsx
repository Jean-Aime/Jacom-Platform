  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative h-[680px] overflow-hidden">
        ...
        {/* Black Gradient Overlay - Same as Homepage */}
        <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/70 to-transparent"></div>
        
        <div className="relative max-w-7xl mx-auto px-6 h-full flex items-center">
          <div className="max-w-3xl">
            <a href="/solutions" className="inline-flex items-center gap-2 text-red-200 hover:text-white text-sm mb-4 transition animate-fade-in-up">
            ...
            {solution.tagline && (
              <p className="text-xs text-red-200 mb-3 uppercase tracking-widest font-medium animate-fade-in-up animation-delay-300">{solution.tagline}</p>
            )}
            <h1 className="text-5xl font-bold text-white mb-6 leading-tight animate-fade-in-up animation-delay-600">{solution.name}</h1>
            <p className="text-red-100 text-lg leading-relaxed mb-8 animate-fade-in-up animation-delay-900">{solution.description}</p>
            <div className="flex gap-4 animate-fade-in-up animation-delay-900">
              <a href="/contact" className="bg-white text-primary px-6 py-3 rounded-lg font-semibold hover:bg-gray-100 transition shadow-lg transform hover:scale-105">
                Request Consultation
              </a>
              <a href="#approach" className="border-2 border-white text-white px-6 py-3 rounded-lg font-semibold hover:bg-white hover:text-primary transition transform hover:scale-105">
                Learn More
              </a>
            </div>
          </div>
            ...
            {solution.benefits && solution.benefits.length > 0 ? solution.benefits.map((benefit: any, i: number) => (
              <div key={i} className="bg-white rounded-xl p-6 text-center hover:shadow-lg transition">
                <div className="w-16 h-16 bg-red-50 rounded-lg flex items-center justify-center mx-auto mb-4">
                  {benefit.icon === 'zap' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>}
                  {benefit.icon === 'target' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                  {benefit.icon === 'bar-chart' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>}
                  {benefit.icon === 'users' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>}
                  {benefit.icon === 'shield' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>}
                  {benefit.icon === 'trending-up' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>}
                  {benefit.icon === 'clock' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                  {benefit.icon === 'award' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg>}
                  {benefit.icon === 'globe' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                  {benefit.icon === 'lightbulb' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>}
                  {benefit.icon === 'dollar-sign' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                  {benefit.icon === 'briefcase' && <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>}
                </div>
            ...
            {solution.implementationSteps && solution.implementationSteps.length > 0 ? solution.implementationSteps.map((step: any, i: number) => (
              <div key={i} className="relative">
                ...
                {i < (solution.implementationSteps.length - 1) && (
                  <div className="hidden md:block absolute top-8 left-full w-full h-0.5 bg-red-100 -ml-4"></div>
                )}
              </div>
            )) : (
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {solution.industries.map((industry: any) => (
                      <a key={industry.id} href={`/industries/${industry.slug}`} className="block p-4 bg-white border border-gray-200 rounded-lg hover:border-primary hover:shadow-md transition group">
                        <h4 className="font-semibold text-gray-900 group-hover:text-primary transition">{industry.name}</h4>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {solution.services.map((service: any) => (
                      <a key={service.id} href={`/services/${service.slug}`} className="block p-4 bg-white border border-gray-200 rounded-lg hover:border-primary hover:shadow-md transition group">
                        <h4 className="font-semibold text-gray-900 group-hover:text-primary transition">{service.name}</h4>
                        {service.description && (
              ...
              <div key={i} className="bg-white rounded-xl p-8 shadow-lg hover:shadow-xl transition">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-primary font-medium">{story.industry}</p>
                  </div>
      ...
      <section className="py-20 bg-gradient-to-r from-primary to-red-700">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <a href="/contact" className="bg-white text-primary px-8 py-4 rounded-lg font-semibold hover:bg-gray-100 transition shadow-lg text-base inline-block">
              Schedule Consultation
            </a>
            <a href="/solutions" className="border-2 border-white text-white px-8 py-4 rounded-lg font-semibold hover:bg-white hover:text-primary transition text-base inline-block">
              View All Solutions
            </a>
          </div>
        </div>
