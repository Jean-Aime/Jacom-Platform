...
class AcademyController {
    ...
    public function enrollCourse($userId) {
        ...
        try {
            ...
            ");
            $stmt->execute([$userId, $data['courseId']]);
            ...
            if ($course && $course['currentEnrolled'] >= $course['maxStudents']) {
                http_response_code(400);
                echo json_encode(['error' => 'This course is full']);
                return;
            }
            ...
            $stmt = $this->conn->prepare("
                INSERT INTO enrollments (id, userId, courseId, location, planType, status, paymentStatus, amountPaid, enrolledAt)
                VALUES (?, ?, ?, ?, ?, 'pending', 'unpaid', 0.00, NOW())
            ");
            ...
            $stmt->execute([
                ...
                $userId,
                $data['courseId'],
                $data['location'] ?? 'Outside Rwanda',
                $data['planType'] ?? 'in_class'
            ]);
            
            // Note: Course enrollment count will be updated when admin approves
            
            // Send enrollment confirmation email
            require_once __DIR__ . '/../services/EmailService.php';
            $emailService = new EmailService();
            $emailService->sendEnrollmentConfirmation($userId, $data['courseId'], $enrollmentId);
            ...
            http_response_code(500);
            echo json_encode(['error' => 'Failed to enroll in course: ' . $e->getMessage()]);
        }
    }
