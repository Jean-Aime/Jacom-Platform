...
export default function CourseViewerPage() {
  ...
  const fetchCourseData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const firstIncomplete = data.lessons.find((l: Lesson) => !l.completed) || data.lessons[0];
  ...
  const useMockData = () => {
    ...
    const mockLessons: Lesson[] = [
      ...
      },
      {
        ...
        completed: false,
        materials: [
          { id: '5', title: 'JavaScript Guide', type: 'pdf', url: '/materials/js.pdf', size: '3.2 MB' }
        ]
      },
      {
        id: '4',
    ...
    setCurrentLesson(mockLessons.find(l => !l.completed) || mockLessons[0]);
  };
  ...
  const markLessonComplete = async (lessonId: string) => {
    try {
      ...
      await fetch(`${backendUrl}/student/lesson/${lessonId}/complete`, {
        method: 'POST',
        headers: {
          'X-Session-Token': token || ''
        },
  ...
  const generateCertificate = async () => {
    try {
      ...
      await fetch(`${backendUrl}/student/course/${course?.id}/certificate`, {
        method: 'POST',
        headers: {
          'X-Session-Token': token || ''
        },
  ...
  const submitQuiz = async () => {
    ...
    if (passed) {
      alert(`Quiz passed with ${percentage}%!`);
      await markLessonComplete(currentLesson.id);
    } else {
      alert(`Quiz score: ${percentage}%. You need ${currentLesson.quiz.passingScore}% to pass.`);
    }
  };
                ...
                <div className="max-w-5xl mx-auto">
                  ...
                  <div className="mt-8 flex items-center justify-between">
                    <button
                      ...
                      className="px-6 py-3 bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-600 text-white rounded-lg font-semibold transition"
                    >
                      ← Previous Lesson
                    </button>
                    <button
                      ...
                      className="px-6 py-3 bg-primary hover:bg-red-700 disabled:bg-gray-800 disabled:text-gray-600 text-white rounded-lg font-semibold transition"
                    >
                      Next Lesson →
                    </button>
                  </div>
                ...
                <div className="mt-8">
                  {!quizSubmitted ? (
                    <button
                      ...
                      className="w-full bg-primary hover:bg-red-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-8 py-3 rounded-lg font-semibold transition"
                    >
                      Submit Quiz
                    </button>
                  ) : (
                    <button
                      ...
                      className="w-full bg-gray-700 hover:bg-gray-600 text-white px-8 py-3 rounded-lg font-semibold transition"
                    >
                      Back to Lesson
                    </button>
                  )}
